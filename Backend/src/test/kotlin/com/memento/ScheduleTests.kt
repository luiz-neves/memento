...
class ScheduleTests : FunSpec({
    ...
    val target = Schedule(repository)
    ...
    test("Get schedule by person ordered by date") {
        ...
        assertSoftly(target.getScheduleByPersonId(1)) {
    ...
    test("Saves what is passed through the API") {
        ...
        val response = target.saveSchedule(ScheduleRequest(1, "event", "2020-02-09 10:00:00", "description", "speaker"))
    ...
    test("Deletes schedule") {
        ...
        val response = target.deleteSchedule(1)
